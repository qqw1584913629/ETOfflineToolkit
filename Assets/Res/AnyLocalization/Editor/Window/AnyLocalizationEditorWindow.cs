...
namespace MH
{
    public class AnyLocalizationEditorWindow : EditorWindow
    {
        ...
        private static void Init()
        {
            ...
            localizationEditorWindow.minSize = new Vector2(800, 800);
            localizationEditorWindow.maxSize = new Vector2(800, 800);
        }
        ...
        private void DrawTextEditingZone()
        {
            EditorGUILayout.BeginVertical("Box");
            {
                ...
                // 添加滚动条
                textEditingScrollViewPos = EditorGUILayout.BeginScrollView(textEditingScrollViewPos, GUILayout.Height(250));
        ...
        private Vector2 textEditingScrollViewPos;
        /// <summary>
        /// 绘制操作栏
        /// </summary>
        private void DrawActionBar()
        {
            EditorGUILayout.BeginHorizontal();
            {
                if (GUILayout.Button("Save to StreamingAssets", GUILayout.MinHeight(22.5f), GUILayout.MinWidth(0)))
                {
        ...
        {
            string streamingAssetsFolder = $"{Application.streamingAssetsPath}/{LocalizationSingleton.XMLStreamingAssetsPath}";
        ...
        {
            string fileName = "Any Localization - " + System.DateTime.Now.ToString("yyyy MM dd") + ".csv";
            string filePath = Path.Combine(Application.dataPath, fileName);
            
            using (StreamWriter writer = new StreamWriter(filePath, false, System.Text.Encoding.UTF8))
            {
                // 写入表头
                List<string> headers = new List<string> { "Key" };
                headers.AddRange(languages.Select(l => l.ToString()));
                writer.WriteLine(string.Join(",", headers));

                // 写入数据
                foreach (var item in strKeyValuePairs)
                {
                    List<string> row = new List<string> { item.Key };
                    foreach (var lang in languages)
                    {
                        item.Value.TryGetValue(lang, out string value);
                        row.Add(value?.Replace(",", "，") ?? string.Empty); // 替换英文逗号为中文逗号，避免CSV格式问题
                    }
        ...
        {
            string csvFile = EditorUtility.OpenFilePanel("请选择要导入的CSV文件", Application.dataPath, "csv");
            ...
            if (headers.Length < 2) return; // 至少需要Key和一种语言
            ...
            for (int i = 1; i < lines.Length; i++)
            {
                ...
                {
                    var lang = (Language)System.Enum.Parse(typeof(Language), headers[j]);
                    string value = j < values.Length ? values[j] : string.Empty;
                    
                    if (kv.ContainsKey(lang))
                        kv[lang] = value;
                    else
                        kv.Add(lang, value);
                }
