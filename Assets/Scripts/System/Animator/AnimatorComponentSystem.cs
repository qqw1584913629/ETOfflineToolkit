...
namespace MH
{
    ...
    public class AnimatorComponentAwakeSystem : AwakeSystem<AnimatorComponent>
    {
        protected override void Awake(AnimatorComponent self)
        {
            ...
            var bridge = gameObject.GetComponent<AnimationEventBridge>();
            if (bridge == null)
            {
                bridge = gameObject.AddComponent<AnimationEventBridge>();
            }
    ...
    public class AnimatorComponentDestroySystem : DestroySystem<AnimatorComponent>
    {
        ...
        {
            // 清除所有动画片段中的事件
            foreach (var clipName in self.animationClips.Keys)
            {
                self.ClearAnimationEvents(clipName);
            }
    ...
    public static class AnimatorComponentSystem
    {
		...
		public static void PlayInTime(this AnimatorComponent self, MotionType motionType, float time)
		{
			...
			{
				Debug.LogError($"motionSpeed数值异常, {motionSpeed}, 此动作跳过");
				return;
			}
        ...
        public static void AddAnimationEvent(this AnimatorComponent self, string clipName, float time, Action callback)
        {
            AnimationClip clip;
            ...
            {
                Debug.LogError($"事件时间 {time} 超出动画 {clipName} 的长度 {clip.length}");
                return;
            }

            string eventId = $"{clipName}_{time}";
            ...
            {
                Debug.LogWarning($"动画 {clipName} 在时间点 {time} 已存在事件，将被覆盖");
        ...
        public static void RemoveAnimationEvent(this AnimatorComponent self, string clipName, float time)
        {
            AnimationClip clip;
            if (!self.animationClips.TryGetValue(clipName, out clip))
            {
            ...
            // 移除回调
            self.EventBridge.eventCallbacks.Remove(eventId);
            
            // 移除动画事件
            AnimationEvent[] events = clip.events;
            var newEvents = events.Where(e => 
                !(e.functionName == "HandleAnimationEvent" && e.stringParameter == eventId)
            ).ToArray();
            clip.events = newEvents;
        }

        public static void ClearAnimationEvents(this AnimatorComponent self, string clipName)
        {
            AnimationClip clip;
            if (!self.animationClips.TryGetValue(clipName, out clip))
            {
            ...
            {
                if (evt.functionName == "HandleAnimationEvent")
                {
                    self.EventBridge.eventCallbacks.Remove(evt.stringParameter);
                }
            }
