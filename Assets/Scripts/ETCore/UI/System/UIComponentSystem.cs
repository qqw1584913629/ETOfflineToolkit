...
namespace MH
{
    ...
    public static class UIComponentSystem
    {
        ...
        public static T GetDlgLogic<T>(this UIComponent self, bool isNeedShowState = false) where T : Entity, IUILogic
        {
            ...
            if (null == baseWindow)
            {
                Debug.LogWarning($"{windowsId} is not created!");
                return null;
            }
            if (!baseWindow.IsPreLoad)
            {
                Debug.LogWarning($"{windowsId} is not loaded!");
                return null;
            }
            ...
            if (isNeedShowState)
            {
                ...
                {
                    Debug.LogWarning($"{windowsId} is need show state!");
                    return null;
                }
            }
        ...
        public static WindowID GetWindowIdByGeneric<T>(this UIComponent self) where T : Entity
        {
            ...
            Debug.LogError($"{typeof(T).FullName} is not have any windowId!");
        ...
        public static void ShowWindow(this UIComponent self, WindowID id, Entity showData = null)
        {
        ...
        /// <typeparam name="T"></typeparam>
        public static void ShowWindow<T>(this UIComponent self, Entity showData = null) where T : Entity, IUILogic
        {
            WindowID windowsId = self.GetWindowIdByGeneric<T>();
        ...
        /// <param name="id"></param>
        /// <param name="showData"></param>
        public static async ETTask ShowWindowAsync(this UIComponent self, WindowID id, Entity showData = null)
        {
        ...
        /// <param name="showData"></param>
        /// <typeparam name="T"></typeparam>
        public static async ETTask ShowWindowAsync<T>(this UIComponent self, Entity showData = null) where T : Entity, IUILogic
        {
            WindowID windowsId = self.GetWindowIdByGeneric<T>();
        ...
        public static void HideWindow(this UIComponent self, WindowID id)
        {
            ...
            {
                Debug.LogWarning($"检测关闭 WindowsID: {id} 失败！");
                return;
            }
            ...
            {
                Debug.LogError($"UIBaseWindow is null  or isDisposed ,  WindowsID: {id} 失败！");
                return;
            }
        ...
        public static void UnLoadWindow(this UIComponent self, WindowID id, bool isDispose = true)
        {
            ...
            if (null == baseWindow)
            {
                Debug.LogError($"UIBaseWindow WindowId {id} is null!!!");
                return;
            }
            UIEventComponent.Instance.GetUIEventHandler(id).BeforeUnload(baseWindow);
            if (baseWindow.IsPreLoad)
            {
                self.Root.GetComponent<ResourcesComponent>().UnLoadAssetSync(baseWindow.UIPrefabGameObject.name);
        ...
        private static UIBaseWindow ReadyToShowBaseWindow(this UIComponent self, WindowID id, Entity showData = null)
        {
            UIBaseWindow baseWindow = self.GetUIBaseWindow(id);
            // 如果UI不存在开始实例化新的窗口
            if (null == baseWindow)
        ...
        private static async ETTask<UIBaseWindow> ShowBaseWindowAsync(this UIComponent self, WindowID id, Entity showData = null)
        {
        ...
        public static void HideAllShownWindow(this UIComponent self, bool includeFixed = false)
        {
            self.IsPopStackWndStatus = false;
            self.UIBaseWindowlistCached.Clear();
            foreach (KeyValuePair<int, UIBaseWindow> windowBase in self.VisibleWindowsDic)
            {
        ...
        {
            var uiPathComponent = self.Root.GetComponent<UIPathComponent>();
            if (!uiPathComponent.WindowPrefabPath.TryGetValue((int)baseWindow.WindowID, out string value))
            {
                Debug.LogError($"{baseWindow.WindowID} uiPath is not Exist!");
                return;
            }

            GameObject go = null;
            var resourcesComponent = self.Root.GetComponent<ResourcesComponent>();
            go = resourcesComponent.LoadAssetSync<GameObject>(value);
        ...
        private static async ETTask LoadBaseWindowsAsync(this UIComponent self, UIBaseWindow baseWindow)
        {
            ...
            {
                Debug.LogError($"{baseWindow.WindowID} is not Exist!");
                return;
            }
        ...
        private static async ETTask PreLoadWindowsItemAsync(this UIComponent self, WindowID windowID)
        {
            ...
            {
                Debug.Log($"开始预加载页面Item  {windowID} -> {poolName}");
